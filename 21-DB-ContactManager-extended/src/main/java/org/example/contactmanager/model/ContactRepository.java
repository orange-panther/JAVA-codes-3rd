...
public class ContactRepository {
    ...
    public void addContact(String name, String phone, String address, ContactType type) {
        String sql = "INSERT INTO contact (name, phone, address, contactType) VALUES(?, ?, ?, ?)";
    ...
    public void updateContact(Contact contact) {
        String sql = "UPDATE contact SET name=?, phone=?, address=?, contactType=? WHERE id=?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            pstmt.setString(1, contact.getName());
            pstmt.setString(2, contact.getPhone());
            pstmt.setString(3, contact.getAddress());
            pstmt.setString(4, contact.getType().toString());
